...
public abstract class Include {
	 ...
	 */
	public static Include fromJSON( JSONObject includeJSON, BuildOptions buildOptions, String fileExtension ) throws JSONException {
		if( includeJSON.has( "file" ) ) {
			return new FileInclude( includeJSON.getString( "file" ), buildOptions );

		} else if( includeJSON.has( "directory" ) ) {
			return new DirectoryInclude( includeJSON.getString( "directory" ), buildOptions, fileExtension );

		} else if( includeJSON.has( "tree" ) ) {
			return new TreeInclude( includeJSON.getString( "tree" ), buildOptions, fileExtension );
